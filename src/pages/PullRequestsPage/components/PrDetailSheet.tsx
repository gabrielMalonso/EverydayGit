...
export const PrDetailSheet: React.FC<PrDetailSheetProps> = React.memo(({
  isOpen,
  onClose,
  detail,
  isLoading,
  ...
  const title = detail ? t('detail.title', { number: detail.number }) : t('list.title');
      ...
      {isLoading && !detail ? (
        ...
        <div className="flex flex-col gap-6">
          ...
          <div className="flex flex-col gap-2">
            ...
            {detail.body?.trim() ? (
              <div className="whitespace-pre-wrap break-words rounded-card-inner border border-border1 bg-surface2/30 p-3 text-sm text-text1 leading-relaxed">
                {detail.body}
              </div>
            ) : (
              ...
              <div className="flex flex-col gap-3">
                {detail.comments.map((comment, i) => (
                  ...
                  >
                    ...
                    <p className="whitespace-pre-wrap break-words text-sm text-text1">{comment.body}</p>
                  </div>
                ))}
              </div>
            )}
...
PrDetailSheet.displayName = 'PrDetailSheet';
...
const ReviewBlock: React.FC<{ review: PrReview; t: (key: string) => string }> = ({ review, t }) => (
  <div className="rounded-card-inner border border-border1 bg-surface2/20 p-3">
    ...
    {review.body?.trim() && (
      <p className="mb-2 whitespace-pre-wrap break-words text-sm text-text1">{review.body}</p>
    )}
