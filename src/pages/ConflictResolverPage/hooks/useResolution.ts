...
export const useResolution = () => {
  ...
  const saveFile = useCallback(
    async (filePath: string, conflictFile: ConflictFile) => {
      ...
      if (!isDemoMode()) {
        await invoke('resolve_conflict_file_cmd', { filePath, resolvedContent, contextKey });
      }
  ...
  const completeMerge = useCallback(async (message?: string) => {
    ...
    return invoke<string>('complete_merge_cmd', { message, contextKey });
  }, [contextKey]);
